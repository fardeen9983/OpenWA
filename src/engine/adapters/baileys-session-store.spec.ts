...
describe('BaileysSessionStore', () => {
  ...
  describe('recordKeyLidMappings (#362)', () => {
    it('learns a lid->pn mapping from a v7 direct-message key', () => {
      store.recordKeyLidMappings({ remoteJid: '111@lid', remoteJidAlt: '628999@s.whatsapp.net' });
      expect(store.resolvePhone('111@lid')).toBe('628999');
    });

    it('learns a lid->pn mapping when the v7 alternate ids are reversed', () => {
      store.recordKeyLidMappings({ remoteJid: '628999@s.whatsapp.net', remoteJidAlt: '111@lid' });
      expect(store.resolvePhone('111@lid')).toBe('628999');
    });

    it('learns a group participant lid->pn mapping from v7 participantAlt', () => {
    ...
    it('ignores a key with no lid/pn pair', () => {
      ...
      store.recordKeyLidMappings({ remoteJid: '333@lid' }); // lid without pn
      expect(store.resolvePhone('333@lid')).toBeNull();
    });
  });
